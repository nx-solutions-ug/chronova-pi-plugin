...
1. **oh-my-pi** (`pi-coding-agent` v17+; resolved to `17.4.2` in `package-lock.json`). The plugin is loaded by oh-my-pi through the `omp.extensions` field in `package.json` (see [package.json](../package.json)).
