...
1. **oh-my-pi** (`pi-coding-agent` v17+; resolved to `17.3.4` in `package-lock.json`). The plugin is loaded by oh-my-pi through the `omp.extensions` field in `package.json` (see [package.json](../package.json)).
